...
function buildPayload(keyword, lang) {
  ...
  const o = hFunc(r);
  const n =
      ''.concat('web').concat(keyword).concat(time).concat(secret).concat(o);
  const sign = hFunc(n);
...
async function query(keyword, lang) {
  const wordsPromise =
      axios.post(BASE_URL, buildPayload(keyword, lang), {headers: HEADERS});
  const sentencesPromise = axios.post(
      BASE_URL, buildPayload('lj:'.concat(keyword), lang), {headers: HEADERS});

  try {
    const [wordsReponse, sentencesResponse] =
        await Promise.all([wordsPromise, sentencesPromise]);
...
async function main() {
  ...
  await writeFile(
      fileName, JSON.stringify(dataExtraction(responseData), null, 2));
}
